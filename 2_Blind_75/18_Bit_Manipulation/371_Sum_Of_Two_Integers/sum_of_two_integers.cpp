...
/*
    ...
    If we were to use XOR only, we would get 1(i.e 0001 in binary), which is
    false.

    So, the problem is--What are we supposed to do when both bits are 1?

    We HAVE TO put 0 as a result of those two bits, but we also need a carry
    that indicates that one bit gets carried further.

    So how are we going to do that?

    If we have two 0s, we definitely don't have a carry.
    If we have one 0,  we also       don't have a carry.

    However, if we have TWO 1s then we DO have a carry.

    So--How do we know if we have TWO 1s?
    It's bitwise "AND" operation.

    If a & b gives us 1, then that means that we have do have a carry.
    However, that 1 has to be added with its left bit.
    ...
    -------                                  |
       1101                                  |
                                             |
                                             v
    2nd from the right(1-based), we have two 1s. However, we have to add that
    carry with two bits 3rd from the right(1-based).

    So how can we ensure that?
        (a & b) << 1

    This means that we're going to "catch" every TWO 1s and Logically shift the
    whole number by one to the left and then we're going to XOR that new number
    with previously XOR-ed value, and so on.

    We're going to have a loop.

    Let's do one example. Note that when we're doing these logical bitwise
    operators--We do NOT have to do them one by one.
    ...
    9 + 11 is definitely not 2, what are we missing?
    We didn't do the AND operation.
    ...
    With "AND" we'll get all the carry bits, but we have to Logically Shift
    those bits to the LEFT because that's how the binary addition works.

    And now we have to add this new number--that is a result of AND operation
    and LSL (Logical Shift Left)--with the previously XOR-ed number.
...
public:
    int getSum(int a, int b)
    {
        while (b != 0)
        {
            ...
            int carry = (a & b) << 1; // carry after LSL (Logical Shift Left)
