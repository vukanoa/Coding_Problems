/*
    ...
    ==========================================================
    FUNCTION: string customSortString(string order, string s);
    ==========================================================
    ...
    should be "c", "b", and "a". Since "d" does not appear in order, it can be
    at any position in the returned string. "dcba", "cdba", "cbda" are also
    valid outputs.
    ...
    so its position is flexible. Following the order of appearance in order,
    "b", "c", and "a" from s should be arranged as "b", "c", "a". "d" can be
    placed at any position since it's not in order. The output "bcad" correctly
    follows this rule. Other arrangements like "dbca" or "bcda" would also be
...
public:
    string customSortString(string order, string s)
    {
        ...
        for (const auto& chr : order)
        {
            if (freq.find(chr) != freq.end())
                result.append(freq[chr], chr);
            freq.erase(chr);
        }
