/*
    ...
    ===============================================
    FUNCTION: int averageOfSubtree(TreeNode* root);
    ===============================================
...
private:
    pair<int,int> postorder_dfs(TreeNode* root, int& result)
    {
        if ( ! root)
            return {0, 0};
        
        auto left  = postorder_dfs(root->left, result);
        auto right = postorder_dfs(root->right, result);
