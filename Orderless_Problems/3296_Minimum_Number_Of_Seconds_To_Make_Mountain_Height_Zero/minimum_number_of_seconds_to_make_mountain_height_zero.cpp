/*
    ...
    worker i:
        ...
        workerTimes[i] * 2 + ... + workerTimes[i] * x seconds.

    For example: To reduce the height of the mountain by 1, it takes
    ...
    =====================================================================================
    FUNCTION: long long minNumberOfSeconds(int mountainHeight, vector<int>& workerTimes);
    =====================================================================================
...
/* Time  Complexity: O(n * logk) */  // 'k' is the number of workers
...
public:
    long long minNumberOfSeconds(int mountainHeight, vector<int>& workerTimes)
    {
        if (workerTimes.size() == 1)
            return workerTimes[0] * static_cast<long long>(mountainHeight) * (mountainHeight + 1) / 2;

         int result = 0;

        // {time, original, height_reduced}
        priority_queue<vector<long long >,
                       vector<vector<long long>>,
                       greater<vector<long long>>> min_heap;

         for (int i = 0; i < workerTimes.size(); i++)
            min_heap.push( {workerTimes[i], workerTimes[i], 1} );

         for (int i = 0; i < mountainHeight; i++)
         {
            long long time           = min_heap.top()[0];
            long long original       = min_heap.top()[1];
            long long height_reduced = min_heap.top()[2];

            min_heap.pop();

            result = time;
            min_heap.push( {time + original * (height_reduced + 1), original, height_reduced + 1} );
         }
