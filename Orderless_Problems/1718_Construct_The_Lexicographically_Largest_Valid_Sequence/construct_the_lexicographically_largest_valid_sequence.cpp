/*
    ...
    ========================================================
    FUNCTION: vector<int> constructDistancedSequence(int n);
    ========================================================
...
public:
    vector<int> constructDistancedSequence(int n)
    {
        ...
        unordered_set<int> set_used;

        backtracking(result, n, set_used, 0);

        return result;
    }

private:
    bool backtracking(vector<int>& result, int n, unordered_set<int>& set_used, int i)
    {
        if (i == result.size())
            return true;
        ...
        for (int num = n; num >= 1; num--)
        {
            ...
            while (j < result.size() && result[j])
                j++; // Iterate until first unpopulated

            // Recursive step
            if (backtracking(result, n, set_used, j))
                return true;
