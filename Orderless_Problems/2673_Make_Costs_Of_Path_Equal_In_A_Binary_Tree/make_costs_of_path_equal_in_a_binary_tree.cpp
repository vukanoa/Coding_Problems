/*
    ...
    ======================================================
    FUNCTION: int minIncrements(int n, vector<int>& cost);
    ======================================================
...
/* Time  Complexity: O(N) */
/* Space Complexity: O(H) */ // Where 'H' is the Height of the Recursion.
class Solution {
public:
...
private:
    void dfs(int node, vector<int>& cost, int& result)
    {
        const int SIZE = cost.size();
        ...
        dfs(L_node, cost, result);
        dfs(R_node, cost, result);

        if (L_node <= SIZE && R_node <= SIZE)
        {
            result  += abs(cost[L_node - 1] - cost[R_node - 1]);
            cost[node - 1] += max(cost[L_node - 1], cost[R_node - 1]);
        }
    }
};
