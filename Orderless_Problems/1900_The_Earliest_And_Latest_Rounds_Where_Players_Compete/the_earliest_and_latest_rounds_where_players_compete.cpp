/*
    ...
    In each round, the ith player from the front of the row competes against
    the ith player from the end of the row, and the winner advances to the next
    ...
    ==================================================================================
    FUNCTION: vector<int> earliestAndLatest(int n, int firstPlayer, int secondPlayer);
    ==================================================================================
...
class Solution {
private:
    int earliest_round_result[30][30][30];
    int latest_round_result[30][30][30];

public:
    vector<int> earliestAndLatest(int n, int firstPlayer, int secondPlayer)
    {
        memset(earliest_round_result, 0, sizeof(earliest_round_result));
        memset(latest_round_result, 0, sizeof(latest_round_result));

        // F(n,f,s) = F(n,s,f)
        if (firstPlayer > secondPlayer)
            swap(firstPlayer, secondPlayer);

        auto [earliest, latest] = compute_rounds(n, firstPlayer, secondPlayer);

        return {earliest, latest};
    }

private:
    pair<int, int> compute_rounds(int n, int first_idx, int second_idx)
    {
        if (earliest_round_result[n][first_idx][second_idx])
            return {earliest_round_result[n][first_idx][second_idx], latest_round_result[n][first_idx][second_idx]};

        if (first_idx + second_idx == n + 1)
            return {1, 1};

        if (first_idx + second_idx > n + 1)
        {
            tie(earliest_round_result[n][first_idx][second_idx], latest_round_result[n][first_idx][second_idx]) = compute_rounds(n, n + 1 - second_idx, n + 1 - first_idx);
            return {earliest_round_result[n][first_idx][second_idx], latest_round_result[n][first_idx][second_idx]};
        }
        ...
        if (second_idx <= n_half)
        {
            // On the left or in the middle
            for (int i = 0; i < first_idx; i++)
            {
                for (int j = 0; j < second_idx - first_idx; j++)
                {
                    auto [x, y] = compute_rounds(n_half, i + 1, i + j + 2);
        ...
        {
            int s_prime = n + 1 - second_idx;

            int mid = (n - 2 * s_prime + 1) / 2;

            for (int i = 0; i < first_idx; i++)
            {
                for (int j = 0; j < s_prime - first_idx; j++)
                {
                    auto [x, y] = compute_rounds(n_half, i + 1, i + j + mid + 2);
        ...
        return {earliest_round_result[n][first_idx][second_idx] = earliest + 1, latest_round_result[n][first_idx][second_idx] = latest + 1};
    }
};
