/*
    ...
    releaseTimes, where releaseTimes[i] was the time the ith key was released.

    Both arrays are 0-indexed. The 0th key was pressed at the time 0, and every
    ...
    =========================================================================
    FUNCTION: char slowestKey(vector<int>& releaseTimes, string keysPressed);
    =========================================================================
...
public:
    ...
    {
        unordered_map<char, int> umap_duration;

        umap_duration[keysPressed[0]] = releaseTimes[0];

        // Find and store the keypress duration for each key in the durationMap
        for (unsigned i = 1; i < releaseTimes.size(); i++)
        {
            int curr_duration = releaseTimes[i] - releaseTimes[i - 1];
            char curr_chr = keysPressed[i];

            umap_duration[curr_chr] =max(umap_duration[curr_chr], curr_duration);
        }

        char slower_key = ' ';
        int longest_press_duration = 0;

        for (const auto& entry : umap_duration)
        {
            char key     = static_cast<char>(entry.first);
            int duration = static_cast<int>(entry.second);

            if (duration > longest_press_duration)
            {
                longest_press_duration = duration;
                slower_key = key;
            }
            else if (duration == longest_press_duration && key > slower_key)
            {
                slower_key = key;
            }
        }
