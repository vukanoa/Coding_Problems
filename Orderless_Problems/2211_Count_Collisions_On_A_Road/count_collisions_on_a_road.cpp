/*
    ...
    be either 'L', 'R', or 'S' denoting whether the ith car is moving towards
    ...
    =================================================
    FUNCTION: int countCollisions(string directions);
    =================================================
    ...
    - Cars 4 and 5 will collide with each other. After car 4 collides with car
       3, it will stay at the point of collision and get hit by car 5. The
       number of collisions becomes 4 + 1 = 5.
    Thus, the total number of collisions that will happen on the road is 5. 

    --- Example 2 ---
    Input: directions = "LLRR"
    Output: 0
    Explanation: No cars will collide with each other. Thus, the total number
                 of collisions that will happen on the road is 0.
...
public:
    int countCollisions(string directions)
    {
        ...
        // Remove leading 'L'
        dir.erase(dir.begin(), find_if(dir.begin(), dir.end(), [](char chr) { return chr != 'L'; }));

        // Remove trailing 'R'
        dir.erase(find_if(dir.rbegin(), dir.rend(), [](char chr) { return chr != 'R'; }).base(), dir.end());

        // Count 'R' and 'L' in the remaining string
        int collisions = 0;
        for (const char& chr : dir)
        {
            if (chr == 'R' || chr == 'L')
                collisions++;
        }
