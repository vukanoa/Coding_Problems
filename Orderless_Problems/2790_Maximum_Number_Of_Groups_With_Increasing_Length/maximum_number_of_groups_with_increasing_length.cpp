/*
    ...
    all groups. You must also satisfy the following conditions:
        ...
        + Each group must consist of distinct numbers, meaning that no
          duplicate numbers are allowed within a single group.

        + Each group (except the first one) must have a length strictly
          greater than the previous group.

    Return an integer denoting the maximum number of groups you can create
    while satisfying these conditions.

    ============================================================
    FUNCTION: int maxIncreasingGroups(vector<int>& usageLimits);
    ============================================================
...
public:
    int maxIncreasingGroups(vector<int>& usageLimits)
    {
        ...
        long long total = 0;
        for (const int& curr_limit : usageLimits)
        {
            total += curr_limit;

            if (total >= (result + 1) * (result + 2) / 2)
                result++;
        }
