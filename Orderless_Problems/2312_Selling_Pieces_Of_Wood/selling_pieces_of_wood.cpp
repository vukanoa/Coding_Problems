/*
    ...
    the entire height or width of the piece to split it into two smaller
    pieces. After cutting a piece of wood into some number of smaller pieces,
    you can sell pieces according to prices. You may sell multiple pieces of
    the same shape, and you do not have to sell all the shapes. The grain of
    the wood makes a difference, so you cannot rotate a piece to swap its
    height and width.
    ...
    ===========================================================================
    FUNCTION: long long sellingWood(int m, int n, vector<vector<int>>& prices);
    ===========================================================================
...
public:
    long long sellingWood(int m, int n, vector<vector<int>>& prices)
    {
        ...
        return solve(m, n, price_table);
    }

private:
    long long solve(int height, int width, int (&price_table)[201][201])
    {
        if (height == 0 || width == 0)
            return 0LL;
        ...
        long long result = price_table[height][width];

        for (int h = 1; h < height; h++)
        {
            result = max(result,   solve(h         , width    , price_table)
                                 + solve(height - h, width    , price_table));
        }

        for (int w = 1; w < width; w++)
        {
            result = max(result,   solve(height    , w        , price_table)
                                 + solve(height    , width - w, price_table));
        }
