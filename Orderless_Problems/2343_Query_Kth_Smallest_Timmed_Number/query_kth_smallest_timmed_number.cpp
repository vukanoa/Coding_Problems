/*
    ...
    ======================================
    2343) Query Kth Smallest Timmed Number
    ======================================
    ...
    =====
    Note:
            + To trim to the rightmost x digits means to keep removing the
              leftmost digit, until only x digits remain.

            + Strings in nums may contain leading zeros.
    =====

    =================================================================================================
    FUNCTION: vector<int> smallestTrimmedNumbers(vector<string>& nums, vector<vector<int>>& queries);
    =================================================================================================
...
public:
    vector<int> smallestTrimmedNumbers(vector<string>& nums, vector<vector<int>>& queries)
    {
        ...
        {
            vector<pair<string,int>> trimmed_with_index;

            for (int i = 0; i < N; i++)
                trimmed_with_index.push_back( {(nums[i].substr(nums[i].size() - q[1])), i} );

            sort(trimmed_with_index.begin(), trimmed_with_index.end());
            result.push_back(trimmed_with_index[q[0] - 1].second);
        }
