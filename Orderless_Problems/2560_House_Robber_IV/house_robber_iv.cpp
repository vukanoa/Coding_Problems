/*
    ...
    ======================================================
    FUNCTION: int minCapability(vector<int>& nums, int k);
    ======================================================
...
/*
    ...
    By-the-book Binary Search.

    If you have a range that the answer is guaranteed to be in, then you can
    perform a Binary Search instead of trying one-by-one until you find the
    one you need.
    ...
    We are GUARANTEED to have at least one solution, therefore go through this
    range and find the minimum one.

    This way we've reduced Time Complexity from O(N * M) down to O(N * logM).
...
/* Time  Complexity: O(N * logM) */ // Where 'M' is max_element in "nums"
...
public:
    int minCapability(vector<int>& nums, int k)
    {
        ...
        int L = 1;                                      // min_reward
        int R = *max_element(nums.begin(), nums.end()); // max_reward
        ...
        while (L < R)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                if (nums[i] <= mid)
                {
                    ...
                    /*
                       Skip the next house to maintain the non-adjacent
                       condition
                    */
                    i++;
                }
