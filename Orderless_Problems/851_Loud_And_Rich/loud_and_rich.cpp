/*
    ...
    person (that is, the person y with the smallest value of quiet[y]) among
    all people who definitely have equal to or more money than the person x.

    ===================================================================================
    FUNCTION: vector<int> loudAndRich(vector<vector<int>>& richer, vector<int>& quiet);
    ===================================================================================
...
// E == Number of people, V = num of relationships
...
public:
    vector<int> loudAndRich(vector<vector<int>>& richer, vector<int>& quiet)
    {
        ...
        for (vector<int>& edge : richer)
        {
            ...
            // Invert the Adjacency List
            adj_list[poorer].push_back(richer);
        }
...
private:
    int dfs(int node, vector<int>& quiet, vector<int>& answer)
    {
        if (answer[node] == -1)
        {
            answer[node] = node;
            ...
            {
                int candidate = dfs(neighbor, quiet, answer);
