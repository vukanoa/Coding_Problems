/*
    ...
    example of one system is represented in the following picture:
        ...
        <Check the picture on LeetCode, I can't include it here. Sorry>
    ...
    ==============================================
    FUNCTION: int lengthLongestPath(string input);
    ==============================================
...
public:
    int lengthLongestPath(string input)
    {
        ...
        unordered_map<int, int> umap_path;
        umap_path[0] = 0;
        ...
        {
            auto name_idx = line.find_last_of('\t') + 1;
            int depth = name_idx;

            string name     = line.substr(name_idx);
            int    name_len = name.length();

            if (name.find('.') != std::string::npos) // It's a file
            {
                result = max(result, umap_path[depth] + name_len);
            }
            else // It's a directory
            {
                umap_path[depth + 1] = umap_path[depth] + name_len + 1;
            }
        }
