...
/*
    ...
    myCalendarTwo.book(5, 10);  // return True, The event can be booked, as it
                                   does not use time 10 which is already double
                                   booked.
    myCalendarTwo.book(25, 55); // return True, The event can be booked, as the
                                   time in [25, 40) will be double booked with
                                   the third event, the time [40, 50) will be
                                   single booked, and the time [50, 55) will be
...
class MyCalendarTwo {
public:
    map<int, int> map_event_count;

    MyCalendarTwo() {}

    bool book(int start, int end) {
        map_event_count[start]++;
        map_event_count[end]--;

        int ongoingEvents = 0;
        for (auto &[time, change] : map_event_count)
        {
