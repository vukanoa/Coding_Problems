/*
    ...
    ===============================================================
    FUNCTION: bool checkDistances(string s, vector<int>& distance);
    ===============================================================
...
public:
    bool checkDistances(string s, vector<int>& distance)
    {
        ...
        int prev_position[26];
        memset(prev_position, 0xff, sizeof(prev_position));

        for (int i = 0; i < N; i++)
        {
            int idx = s[i] - 'a';
            if (prev_position[idx] == -1)
            {
                prev_position[idx] = i; // first occurrence
            }
            else
            {
                int letters_between = i - prev_position[idx] - 1;
                if (letters_between != distance[idx])
                    return false;
            }
        }
