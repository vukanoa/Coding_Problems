...
/*
    ...
    =============================================================
    FUNCTION: int timeRequiredToBuy(vector<int>& tickets, int k); 
    =============================================================
    ...
    The person at position 2 has successfully bought 2 tickets and it took 3 +
    3 = 6 seconds.
...
public:
    ...
    {
        int result = 0;
        int kth_desire = tickets[k];

        for (int i = 0; i <= k; i++)
            result += min(kth_desire, tickets[i]);
        
        for (int i = k+1; i < tickets.size(); i++)
            result += min(kth_desire - 1, tickets[i]);
