/*
    ...
    You are given an array tasks where tasks[i] = [actuali, minimumi]:
        ...
        + actuali is the actual amount of energy you spend to finish the ith
          task.
    ...
    ========================================================
    FUNCTION: int minimumEffort(vector<vector<int>>& tasks);
    ========================================================
...
/* Space Complexity: O(logN)     */ // Space Complexity of C++'s Into Sort
class Solution {
public:
    int minimumEffort(vector<vector<int>>& tasks)
    {
        ...
        /* Sort in DESCENDING order by the difference of (minimal - actual)*/
        ...
        int balance = tasks[0][1] - tasks[0][0];
        int loan    = 0;

        for (int i = 1; i < N; i++)
        {
            int actual_energy  = tasks[i][0];
            int minimal_energy = tasks[i][1];

            if (balance < minimal_energy)
            {
                loan += minimal_energy - balance;

                balance = minimal_energy;
            }

            balance -= actual_energy;
        }
