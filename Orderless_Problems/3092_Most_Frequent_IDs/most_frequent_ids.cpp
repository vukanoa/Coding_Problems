/*
    ...
    removed from the collection at each step.
        ...
        + Addition of IDs: If freq[i] is positive, it means freq[i] IDs with
          the value nums[i] are added to the collection at step i.

        + Removal of IDs: If freq[i] is negative, it means -freq[i] IDs with
          the value nums[i] are removed from the collection at step i.
    ...
    ==================================================================================
    FUNCTION: vector<long long> mostFrequentIDs(vector<int>& nums, vector<int>& freq);
    ==================================================================================
...
public:
    vector<long long> mostFrequentIDs(vector<int>& nums, vector<int>& freq)
    {
        ...
        vector<long long> result(N, LLONG_MIN);

        unordered_map<long long, long long> counter;
        map<long long, unordered_set<long long>> map_uset;
        ...
        for (int i = 0; i < N; i++)
        {
            if (freq[i] > 0)
            {
                ...
                {
                    long long  curr_freq = counter[nums[i]];
                    long long  new_freq  = curr_freq + freq[i];

                    map_uset[curr_freq].erase(nums[i]);
                    if (map_uset[curr_freq].empty())
                        map_uset.erase(curr_freq);

                    map_uset[new_freq].insert(nums[i]);

                    counter[nums[i]] = new_freq;
                }
                else
                {
                    counter[nums[i]] = freq[i];
                    map_uset[freq[i]].insert(nums[i]);
                }
            }
            else
            {
            ...
            auto top = map_uset.rbegin();    // Reversed iterator points to the largest key
            result[i] = top->first;
        }
