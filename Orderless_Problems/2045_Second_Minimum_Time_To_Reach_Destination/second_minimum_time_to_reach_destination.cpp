...
/*
    ...
    =====
    Note: You can go through any vertex any number of times, including 1 and n.
          You can assume that when the journey starts, all signals have just
          turned green.
    =====

    =====================================================================================
    FUNCTION: int secondMinimum(int n, vector<vector<int>>& edges, int time, int change);
    =====================================================================================
...
public:
    int secondMinimum(int n, vector<vector<int>>& edges, int time, int change)
    {
        ...
        vector<int> dist_1(n+1, INT_MAX);
        vector<int> dist_2(n+1, INT_MAX);

        queue<pair<int, int>> queue; // {node, distance}
        queue.emplace(1, 0);
        dist_1[1] = 0;
        ...
        while ( ! queue.empty())
        {
            ...
            for (int nei : adj_list[node])
            {
                ...
                if (new_distance < dist_1[nei])
                {
                    dist_2[nei] = dist_1[nei];
                    dist_1[nei]  = new_distance;
                    queue.emplace(nei, new_distance);
                }
                else if (new_distance > dist_1[nei] && new_distance < dist_2[nei])
                {
                    dist_2[nei] = new_distance;
                    queue.emplace(nei, new_distance);
                }
            }
        }
