/*
    ...
    represents the number of ways to select a total amount i using an infinite
    supply of some fixed coin denominations. Each denomination is a positive
    integer with value at most numWays.length.
    ...
    ======================================================
    FUNCTION: vector<int> findCoins(vector<int>& numWays);
    ======================================================
...
public:
    vector<int> findCoins(const vector<int>& numWays)
    {
        ...
        {
            int target = numWays[amount-1];

            if (dp[amount] == 1LL*target - 1)
            {
