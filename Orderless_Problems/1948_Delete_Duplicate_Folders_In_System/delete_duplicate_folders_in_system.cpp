/*
    ...
    well as all their subfolders.
        ...
        folders "/a" and "/b" would not be identical. Note that "/a/x" and
    ...
    ======================================================================================
    FUNCTION: vector<vector<string>> deleteDuplicateFolder(vector<vector<string>>& paths);
    ======================================================================================
    ...
    because they both contain an empty folder named "b".
    ...
    because they both contain an empty folder named "y". Note that folders
    "/a" and "/c" are identical after the deletion, but they are not deleted
...
/* Time  Complexity: O(N * L * logL) */ // Where L is the average folder depth
...
private:
    ...
    unordered_map<string, int> serial_frequency;
...
public:
    vector<vector<string>> deleteDuplicateFolder(vector<vector<string>>& paths)
    {
        ...
        construct_serials(root);
        collect_valid_paths(root);

        return result;
    }

private:
    void construct_serials(TrieNode* node)
    {
        ...
        {
            const string& folder = entry.first;
            TrieNode* child = entry.second;

            construct_serials(child);
            serialized_children.push_back(folder + "(" + child->serial + ")");
        }

        sort(serialized_children.begin(), serialized_children.end());

        for (const string& s : serialized_children)
            node->serial += s;

        serial_frequency[node->serial]++;
    }

    void collect_valid_paths(TrieNode* node)
    {
        if (serial_frequency[node->serial] > 1)
            return;
        ...
        for (auto& entry : node->children)
        {
            ...
            curr_path.push_back(folder);
            collect_valid_paths(child);
            curr_path.pop_back();
        }
    }
};
