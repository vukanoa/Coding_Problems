/*
    ...
    =========================================
    1275) Find a Winner of a Tic Tac Toe Game
    =========================================
    ...
    (i.e., it follows the rules of Tic-Tac-Toe), the grid is initially empty,
    and A will play first.

    =======================================================
    FUNCTION: string tictactoe(vector<vector<int>>& moves);
    =======================================================
...
public:
    string tictactoe(vector<vector<int>> &moves)
    {
        ...
        for (int i = 0; i < number_of_moves; i++)
        {
            ...
            int player_value = (i % 2 == 0) ? 1 : -1;

            rows[row] += player_value;
            cols[col] += player_value;

            if (row == col)
                diagonal_1 += player_value;

            if (row + col == N - 1)
                diagonal_2 += player_value;

            if (abs(rows[row])  == N || abs(cols[col])  == N ||
                abs(diagonal_1) == N || abs(diagonal_2) == N)
            {
                return player_value == 1 ? "A" : "B";
            }
        }

        return number_of_moves == 9 ? "Draw" : "Pending";
    }
};
