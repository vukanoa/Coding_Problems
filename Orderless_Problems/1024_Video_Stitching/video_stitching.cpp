/*
    ...
    time seconds. These video clips can be overlapping with each other and have
    varying lengths.
    ...
    ===================================================================
    FUNCTION: int videoStitching(vector<vector<int>>& clips, int time);
    ===================================================================
...
public:
    int videoStitching(vector<vector<int>>& clips, int T) 
    {
        ...
        int idx       = 0;
        int max_reach = 0; // Max reach TIME

        while (max_reach < T)
        {
            int curr_max_reach = 0;

            while (idx < N && clips[idx][0] <= max_reach)
                curr_max_reach = max(curr_max_reach, clips[idx++][1]);
