/*
    ...
    Their cycles are synchronized: every traffic light starts a new cycle at
    the same time, and every cycle lasts exactly period seconds. Therefore, the
    red phase of the ith traffic light lasts for period - lights[i] seconds.

    You are also given an integer array arrivalTime, where arrivalTime[j] is
    the arrival time, in seconds, of the jth car.

    Each car must be assigned to exactly one traffic light. Multiple cars may
    be assigned to the same traffic light. Any number of cars may cross the
    same traffic light simultaneously while it is green. Cars do not block or
    delay one another.

    For a car j assigned to the ith traffic light, let r = arrivalTime[j] %
    period. If r < lights[i], its waiting time is 0. Otherwise, its waiting
    time is period - r.
    ...
    ====================================================================================
    FUNCTION: int minPenalty(int period, vector<int>& lights, vector<int>& arrivalTime);
    ====================================================================================
...
/*
    ...
    Realizing that we can use the same light for ALL of the cars is the key.
    Simply take the light with the longest green phase and the problem is over.
...
public:
    int minPenalty(int period, vector<int>& lights, vector<int>& arrivalTime)
    {
        ...
        const int M = arrivalTime.size();
        int result = 0;

        int max_green_phase = *max_element(lights.begin(), lights.end());

        for (int j = 0; j < M; j++)
        {
            int r = arrivalTime[j] % period;

            if (r >= max_green_phase)
