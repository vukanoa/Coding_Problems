/*
    ...
    represents the x-coordinate, y-coordinate, and travel range of the ith
    drone.

    You are also given an integer array target = [tx, ty], representing the
    coordinates of the target.

    A drone drones[i] can reach the target if the between its coordinates and
    the target coordinates is less than or equal to its rangei.

    Return the index of the reachable drone with the minimum Manhattan distance
    to the target. If there is a tie, return the smallest index. If no drone
    can reach the target, return -1.

    ============================================================================
    FUNCTION: int nearestDrone(vector<vector<int>>& drones, vector<int>& target);
    ============================================================================
...
public:
    int nearestDrone(vector<vector<int>>& drones, vector<int>& target)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            int manhattan =  abs(drones[i][0] - target[0]) +
                             abs(drones[i][1] - target[1]);

            if (manhattan <= drones[i][2] && manhattan < min_distance)
            {
                result = i;
