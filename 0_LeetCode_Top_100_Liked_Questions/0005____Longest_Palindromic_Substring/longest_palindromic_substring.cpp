...
class Solution_another_N_Squared {
public:
    std::string longestPalindrome(std::string s)
    {
        ...
        for (int x = 0; x < N; x++)
        {
            ...
            for (int i = x-1, j = x+1; i >= 0 && j < N; i--, j++)
            {
                if (s[i] != s[j])
                    break;
            ...
            if (longest < palindrome_len)
            {
                longest = palindrome_len;
                palindrome = s.substr(x - palindrome_len/2, palindrome_len);
            }
        }
        ...
        for (int x = 0; x < N-1; x++)
        {
            ...
            {
                longest = palindrome_len;
                palindrome = s.substr(x - palindrome_len/2 + 1, palindrome_len);
            }
        }
...
main()
{
    ...
    // Solution_another_N_Squared    sol_another_n2;  // O(n^2)
    ...
    // cout << "\n\n\n\t--- Longest palindromic substring --- \n\t\"" << sol_another_n2.longestPalindrome(s) << "\"";  // O(n^2)
