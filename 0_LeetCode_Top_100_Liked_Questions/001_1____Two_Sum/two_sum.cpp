...
public:
    std::vector<int> twoSum(std::vector<int>& nums, int target)
    {
        ...
        map.insert({nums[0], 0}); // Insert 0-th element

        for (int i = 1; i < nums.size(); i++) // We start from index 1
        {
...
public:
    std::vector<int> twoSum(std::vector<int>& nums, int target)
    {
        ...
        for (int i = 0; i < nums.size(); i++)
        {
            ...
            umap.insert({nums[i], i}); // umap[nums[i]] = i;
        }
...
main()
{
    ...
    std::vector<int> solution_vect = sol_2.twoSum(nums, target);
