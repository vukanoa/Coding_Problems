...
public:
    bool canFinish(int numCourses, std::vector<std::vector<int>>& prerequisites)
    {
        std::unordered_map<int, std::vector<int>> adj_list(numCourses);
...
private:
    bool dfs(std::unordered_map<int, std::vector<int>> & adj_list,
             ...
             int course)
    {
        if (path[course]) // Loop detected
            return false;

        path[course] = true;

        for (const int& prereq : adj_list[course])
        {
            if (taken.find(prereq) != taken.end()) // "prepreq" already taken
                continue;

            if (!dfs(adj_list, taken, path, prereq))
                return false;
        }
...
main()
{
    ...
    if (sol_clean.canFinish(numCourses, prerequisites))
