...
/*
    ...
    It's almost equivalent to the above Solution, however it uses a vector
    instead of an array on the stack, therefore it's a bit less efficient, but
    it's more verbose and easier to read.

    It's "weird" to see this line up above:

            dp[a] = amount + 1;

    It makes much more sense and it's more natural to initialize vector dp with
    a value that is right above the constraint of "amount".

    Hence, these two lines:
        const int over_max_amount = 10001;
        std::vector<int> dp(amount + 1, over_max_amount);
...
public:
    int coinChange(std::vector<int>& coins, int amount)
    {
        ...
        for (int am = 1; am <= amount; am++)
        {
            ...
            {
                if (am - coin >= 0)
                    dp[am] = std::min(dp[am], 1 + dp[am - coin]);
                else
                    break;
            }
        }

        return dp[amount] == over_max_amount ? -1 : dp[amount];
    }
};
...
main()
{
    ...
    // int number = sol_2.coinChange(nums, amount);
