...
/*
    ...
    *** Constraints ***
    ...
    getAverageTime.  Answers within 10-5 of the actual value will be accepted.
...
public:
    ...
    {
        // O(1)
        umap_checkIn[id] = std::make_pair(stationName, t);
    }

    void checkOut(int id, std::string stationName, int t)
    {
        // O(1)
        std::string startStation = umap_checkIn[id].first;
        int t_start = umap_checkIn[id].second;

        umap_checkIn.erase(id);

        /* For ease of readability */
        std::string endStation = stationName;
        int t_end = t;

        int travel_time = t_end - t_start;

        // O(1)
        umap_average[startStation][endStation].first += travel_time;
        umap_average[startStation][endStation].second++;
    }

    double getAverageTime(std::string startStation, std::string endStation)
    {
        int summed_up_times = umap_average[startStation][endStation].first;
        int total_travels   = umap_average[startStation][endStation].second;

        // O(1)
        double average = 1.0 * summed_up_times / total_travels;

        return average;
    }

private:
    std::unordered_map<int, std::pair<std::string, int>> umap_checkIn;
    std::unordered_map<std::string, std::unordered_map<std::string, std::pair<int, int>>> umap_average;
};
