...
/*
    ...
    seconds everytime you're coming back from a subtree that has an apple in
    some tree down the subtree. (It can be the root of that subtree as well)
...
public:
    int minTime(int n, vector<vector<int>>& edges, vector<bool>& hasApple)
    {
        ...
        int seconds = 0;
        dfs(adj_list, hasApple, 0, seconds, -1);

        return seconds;
    }

private:
    bool dfs(std::unordered_map<int, std::vector<int>>& adj_list, std::vector<bool>& hasApple,
             int curr_node, int& seconds, int prev)
    {
        bool apple = false;
        ...
        for (int& neighbor : adj_list[curr_node])
        {
            ...
            if (dfs(adj_list, hasApple, neighbor, seconds, curr_node))
                apple = true;
        }
