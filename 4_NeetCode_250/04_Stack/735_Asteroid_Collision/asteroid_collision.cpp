...
/*
    ...
    (Note that we're using a "vector" instead of a "stack, since we can easily
     ...
     vector at the end. Therefore, we'll only use a vector as a stack and
...
/*
    ...
    Another way of implementing it. This was is more straightforward in my
    opinion.
...
public:
    vector<int> asteroidCollision(vector<int>& asteroids)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            if (asteroids[i] > 0) // POSITIVE ones CANNOT collide with previous
            {
                stack.push_back(asteroids[i]);
                continue;
            }

            // Pop as long as the ones on top are POSITIVE and STRICTLY SMALLER
