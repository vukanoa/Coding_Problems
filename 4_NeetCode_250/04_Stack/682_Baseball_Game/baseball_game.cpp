/*
    ...
    operation you must apply to the record and is one of the following:
        ...
        An integer x.  Record a new score of x.  '+'.  Record a new score that
        is the sum of the previous two scores.  'D'.  Record a new score that
        is the double of the previous score.  'C'.  Invalidate the previous
        score, removing it from the record.
    ...
    ====================================================
    FUNCTION: int calPoints(vector<string>& operations);
    ====================================================
...
public:
    int calPoints(vector<string>& operations)
    {
        ...
        for (const string& token : operations)
        {
            if (token == "+")
            {
                /*
                   We needed to pop only the very top one, and we could use
                   stack.top() for the second one, however I think this is
                   easier to read and there's no real time-draining.
                */
